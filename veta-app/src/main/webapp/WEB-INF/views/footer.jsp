...
<style>
  ...
  #veta-chat-bubble-label{
    background:#fff; color:var(--veta-navy); font-weight:700; font-size:13px;
    padding:8px 16px; border-radius:20px; box-shadow:0 3px 10px rgba(0,0,0,.18);
    white-space:nowrap; animation:veta-float 3s ease-in-out infinite;
  }
  #veta-chat-fab{
    width:58px; height:58px; border-radius:50%; background:var(--veta-navy);
    display:flex; align-items:center; justify-content:center; cursor:pointer;
    box-shadow:0 4px 14px rgba(0,0,0,.3); border:3px solid var(--veta-gold);
    font-size:24px; color:#fff; animation:veta-float 3s ease-in-out infinite;
    transition:transform .15s;
  }
  #veta-chat-fab:hover{ transform:scale(1.07); }
  @keyframes veta-float{ 0%,100%{transform:translateY(0);} 50%{transform:translateY(-6px);} }

  #veta-chat-panel{
    position:fixed; bottom:100px; right:26px; width:320px; max-width:88vw;
    height:440px; max-height:70vh; background:var(--veta-navy); color:#fff;
    border-radius:14px; box-shadow:0 10px 30px rgba(0,0,0,.35); z-index:99999;
    display:none; flex-direction:column; overflow:hidden;
    font-family:'Inter','Segoe UI',Arial,sans-serif;
  }
  #veta-chat-panel.veta-open{ display:flex; }

  #veta-chat-header{
    background:var(--veta-navy-dark); padding:12px 14px; display:flex;
    align-items:center; gap:10px; border-bottom:3px solid var(--veta-gold);
  }
  #veta-chat-header .veta-bot-icon{
    width:32px; height:32px; border-radius:50%; background:var(--veta-gold);
    display:flex; align-items:center; justify-content:center; font-size:16px; flex-shrink:0;
  }
  #veta-chat-header .veta-title{ font-weight:700; font-size:14px; }
  #veta-chat-header .veta-sub{ font-size:10.5px; opacity:.75; }
  #veta-chat-close{
    margin-left:auto; background:none; border:none; color:#fff;
    font-size:18px; cursor:pointer; opacity:.8; line-height:1;
  }
  #veta-chat-close:hover{ opacity:1; }

  #veta-chat-body{
    flex:1; overflow-y:auto; padding:12px; display:flex;
    flex-direction:column; gap:8px;
  }
  #veta-chat-body::-webkit-scrollbar{ width:6px; }
  #veta-chat-body::-webkit-scrollbar-thumb{ background:rgba(255,255,255,.2); border-radius:4px; }

  .veta-msg{ max-width:88%; padding:9px 12px; border-radius:12px; font-size:13px; line-height:1.45; }
  .veta-msg-bot{
    background:#fff; color:#1a1a1a; align-self:flex-start;
    border-bottom-left-radius:2px; white-space:pre-line;
  }
  .veta-msg-user{
    background:var(--veta-gold); color:var(--veta-navy-dark);
    align-self:flex-end; border-bottom-right-radius:2px; font-weight:600;
  }

  #veta-faq-list{
    display:flex; flex-direction:column; gap:6px; padding:0 12px 8px;
    max-height:120px; overflow-y:auto;
  }
  .veta-faq-btn{
    background:rgba(255,255,255,.08); border:1px solid rgba(255,255,255,.25);
    color:#fff; text-align:left; padding:7px 10px; border-radius:8px;
    font-size:12px; cursor:pointer; transition:background .15s;
  }
  .veta-faq-btn:hover{ background:var(--veta-gold); color:var(--veta-navy-dark); border-color:var(--veta-gold); }

  #veta-chat-input-row{ display:flex; gap:8px; padding:10px; background:var(--veta-navy-dark); }
  #veta-chat-input{ flex:1; border:none; border-radius:20px; padding:8px 13px; font-size:13px; outline:none; }
  #veta-chat-send{
    background:var(--veta-gold); color:var(--veta-navy-dark); border:none;
    border-radius:50%; width:34px; height:34px; cursor:pointer; font-size:14px; flex-shrink:0;
  }
  #veta-chat-send:hover{ background:var(--veta-gold-dark); }

  @media (max-width:600px){
    #veta-chat-panel{ right:12px; bottom:92px; width:90vw; }
    #veta-chat-launcher{ right:16px; bottom:16px; }
    #veta-chat-bubble-label{ display:none; }
  }
</style>

<div id="veta-chat-launcher" onclick="vetaChatToggle()">
  <div id="veta-chat-bubble-label"><% if(sw){ %>Kozi &amp; Msaada 💬<% } else { %>Courses &amp; Help 💬<% } %></div>
  <div id="veta-chat-fab"><span id="veta-fab-icon">💬</span></div>
</div>
...
<div id="veta-chat-panel">
  <div id="veta-chat-header">
    ...
    <div>
      <div class="veta-title"><% if(sw){ %>Msaidizi wa KIGOMA RVTSC<% } else { %>KIGOMA RVTSC Assistant<% } %></div>
      <div class="veta-sub"><% if(sw){ %>Uliza swali lolote kuhusu chuo<% } else { %>Ask anything about the college<% } %></div>
    </div>
    <button id="veta-chat-close" onclick="vetaChatToggle()">✕</button>
  </div>

  <div id="veta-chat-body">
    <div class="veta-msg veta-msg-bot"><% if(sw){ %>Karibu KIGOMA RVTSC! 👋 Mimi ni msaidizi wako wa maswali ya haraka. Chagua swali hapa chini au andika swali lako.<% } else { %>Welcome to KIGOMA RVTSC! 👋 I'm your quick-answers assistant. Pick a question below or type your own.<% } %></div>
  </div>

  <div id="veta-faq-list"></div>

  <div id="veta-chat-input-row">
    <input type="text" id="veta-chat-input" placeholder="<% if(sw){ %>Andika swali lako...<% } else { %>Type your question...<% } %>" onkeypress="if(event.key==='Enter') vetaChatSend()">
